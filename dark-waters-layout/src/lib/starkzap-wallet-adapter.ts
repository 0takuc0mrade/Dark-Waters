...
const sharedRpcProvider = new RpcProvider({ nodeUrl: SEPOLIA_CONFIG.RPC_URL })
...
export class StarkzapAccountWallet extends BaseWallet {
  ...
  constructor(account: any, provider: any = sharedRpcProvider) {
  ...
  async ensureReady(_options?: EnsureReadyOptions): Promise<void> {
    ...
    if (!deployed) {
      throw new Error("Connected account is not deployed yet.")
    }
  }
